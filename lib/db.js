...
async function query(method, path, body = null) {
  // Cek dulu apakah env variable sudah diisi
  ...
  // Kalau response tidak OK, tampilkan pesan error yang jelas
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Supabase error: ${res.status} — ${err}`);
  }

  // Baca response dengan aman (hindari error saat body kosong)
