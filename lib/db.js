...
// Helper: selalu simpan hwids sebagai JSON string (karena kolom Supabase bertipe text)
function serializeHwids(hwids) {
  if (!hwids) return '[]';
  if (Array.isArray(hwids)) return JSON.stringify(hwids);
  if (typeof hwids === 'string') {
    try { JSON.parse(hwids); return hwids; } catch { return '[]'; }
  }
  return '[]';
}

// Helper: selalu baca hwids sebagai array JS
function parseHwids(hwids) {
...
async function saveLicense(licData) {
  const row = {
    ...
    status: 'unused',
    hwid: null,
    hwids: serializeHwids([]),   // FIX: simpan sebagai JSON string "[]"
...
async function activateLicense(key, hwid) {
  ...
  const currentHwids = parseHwids(existing.hwids); // FIX: parse dulu sebelum pakai
  ...
  const data = await query('PATCH', `licenses?key=eq.${encodeURIComponent(key)}`, {
    hwid: updatedHwids[0],
    hwids: serializeHwids(updatedHwids), // FIX: simpan sebagai JSON string
    status,
...
async function updateLicense(key, updates) {
  // FIX: kalau ada hwids di updates, pastikan disimpan sebagai JSON string
  if (updates.hwids !== undefined) {
    updates.hwids = serializeHwids(updates.hwids);
  }
