...
function ensureArray(hwids) {
  if (Array.isArray(hwids)) return hwids;
  if (!hwids) return [];
  if (typeof hwids === 'string') {
    try { return JSON.parse(hwids); } catch { return []; }
  }
  return [];
}
...
async function activateLicense(key, hwid) {
  ...
  const currentHwids = ensureArray(existing.hwids);
...
async function updateLicense(key, updates) {
  if (updates.hwids !== undefined) {
    updates.hwids = ensureArray(updates.hwids);
  }
...
// ============================================================
// Fungsi tambahan: license_users
// ============================================================

async function getLicenseUsers(licenseKey) {
  const data = await query(
    'GET',
    `license_users?license_key=eq.${encodeURIComponent(licenseKey)}&is_active=eq.true&select=id,username,display_name,role&order=display_name.asc`
  );
  return data || [];
}

async function addLicenseUser({ license_key, username, display_name, pin_hash, role }) {
  const data = await query('POST', 'license_users', {
    license_key,
...
async function updateLicenseUserPin(licenseKey, username, pin_hash) {
  await query(
    'PATCH',
    `license_users?license_key=eq.${encodeURIComponent(licenseKey)}&username=eq.${encodeURIComponent(username)}`,
    { pin_hash }
  );
}

async function deactivateLicenseUser(licenseKey, username) {
  await query(
    'PATCH',
    `license_users?license_key=eq.${encodeURIComponent(licenseKey)}&username=eq.${encodeURIComponent(username)}`,
    { is_active: false }
...
module.exports = {
  ...
  getAllLicenses,
  // Fungsi baru untuk license_users
  getLicenseUsers,
  addLicenseUser,
  updateLicenseUserPin,
  deactivateLicenseUser,
};
