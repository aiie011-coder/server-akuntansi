...
export async function saveLicense(licData) {
  const row = {
    ...
    status: 'unused',
    hwid: null,          // kolom lama — tetap diisi untuk kompatibilitas
    hwids: [],           // kolom baru — array perangkat terdaftar
...
export async function activateLicense(key, hwid) {
  ...
  // Normalisasi: pastikan hwids selalu array
  const currentHwids = Array.isArray(existing.hwids) ? existing.hwids : [];
  const maxDevices = existing.max_devices || 1;

  // Jika hwid sudah terdaftar, tidak perlu update — langsung return
  if (currentHwids.includes(hwid)) {
    return existing;
  }

  // Cek apakah sudah mencapai batas perangkat
  ...
  const data = await query('PATCH', `licenses?key=eq.${encodeURIComponent(key)}`, {
    hwid: updatedHwids[0],        // kolom lama — tetap update untuk kompatibilitas
    hwids: updatedHwids,          // kolom baru
    status,
...
// ============================================================
// Fungsi tambahan untuk sistem plan/lisensi
// ============================================================
...
export async function updateLicenseLastSeen(key, { hwid, last_seen }) {
  await query('PATCH', `licenses?key=eq.${encodeURIComponent(key)}`, {
    last_seen,
    hwid: hwid || undefined,
  });
...
// ============================================================
// CommonJS exports — agar bisa dipakai dengan require()
// ============================================================
module.exports = {
  getLicense,
  saveLicense,
