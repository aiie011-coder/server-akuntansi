...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="cb">
      <div class="form-row">
        <div class="fg"><label>Nama Pembeli</label><input class="fc" id="g-name" placeholder="cth: PT Maju Jaya"></div>
        ...
        <div class="fg"><label>Jumlah</label><input class="fc" id="g-count" type="number" value="1" min="1" max="20"></div>
        <div class="fg"><label>Catatan</label><input class="fc" id="g-note" placeholder="cth: via Tokopedia"></div>
      </div>
      ...
      <div class="gen-result" id="gen-result">
        ...
        <div id="gen-keys" style="font-family:var(--mono);font-size:15px;letter-spacing:2px;color:#047857;line-height:2.2;"></div>
        <button class="btn btn-s btn-sm" style="margin-top:8px;" onclick="copyKeys()">📋 Salin Semua</button>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>#</th><th>Kode Lisensi</th><th>Nama</th><th>Tipe</th><th>Status</th><th>Aktivasi</th><th>Berlaku S/d</th><th>Catatan</th><th>Aksi</th></tr></thead>
        <tbody id="lic-tbody"><tr><td colspan="9" style="text-align:center;padding:32px;color:var(--text3);">⏳ Memuat...</td></tr></tbody>
      </table>
    </div>
...
async function api(action, data={}, key=null) {
...
// Override: gunakan session cookie bukan Bearer token
...
async function loadLicenses() {
  try {
    ...
    document.getElementById('lic-tbody').innerHTML =
      '<tr><td colspan="9" style="text-align:center;padding:24px;color:var(--danger);">❌ Gagal memuat: ' + e.message + '</td></tr>';
  }
}
...
function filterTable() {
  ...
  document.getElementById('lic-tbody').innerHTML = filtered.map((l,i)=>\`
    <tr>
      ...
      <td><span class="mono" style="font-weight:700;">\${l.key}</span>
        <button onclick="copyKey('\${l.key}')" style="background:none;border:none;cursor:pointer;" title="Salin">📋</button></td>
      ...
      <td><div style="display:flex;gap:4px;">
        \${l.status==='aktif'||l.status==='trial'?\`<button class="btn btn-d btn-sm" onclick="setStatus('\${l.key}','nonaktif')">🚫</button>\`:''}
        \${l.status==='nonaktif'?\`<button class="btn btn-s btn-sm" onclick="setStatus('\${l.key}','aktif')">✅</button>\`:''}
        \${l.hwid?\`<button class="btn btn-sm" style="background:var(--bg);border:1.5px solid var(--border);" onclick="doReset('\${l.key}')">🔄</button>\`:''}
        <button class="btn btn-d btn-sm" onclick="doDelete('\${l.key}')">🗑️</button>
      </div></td>
    </tr>
...
function typeBadge(t){const m={lifetime:'♾️ Seumur Hidup','1year':'📅 1 Tahun','6month':'📅 6 Bulan',trial:'⏰ Trial'};return '<span style="font-size:12px;">'+(m[t]||t)+'</span>';}
function statusBadge(s){const m={unused:'<span class="bx b-unused">⬜ Belum Aktif</span>',aktif:'<span class="bx b-aktif">✅ Aktif</span>',trial:'<span class="bx b-trial">⏰ Trial</span>',nonaktif:'<span class="bx b-nonaktif">🚫 Nonaktif</span>'};return m[s]||s;}
...
async function generateLicense() {
  ...
  try {
    const res = await apiCall('generate',{name,type,note,count});
    if (res.success) {
      lastKeys = res.keys;
      document.getElementById('gen-result').style.display='block';
      document.getElementById('gen-keys').innerHTML = res.keys.map(k=>\`<div>🔑 <strong>\${k}</strong></div>\`).join('');
...
function copyKeys(){navigator.clipboard.writeText(lastKeys.join('\\n')).then(()=>toast('📋 Disalin!','i'));}
function copyKey(k){navigator.clipboard.writeText(k).then(()=>toast('📋 Kode disalin!','i'));}

async function setStatus(key,status){
  try{await apiCall('updateStatus',{status},key);toast('Status diubah → '+status,status==='aktif'?'s':'e');loadLicenses();}
  catch(e){toast('❌ '+e.message,'e');}
}
async function doReset(key){
  if(!confirm('Reset HWID untuk '+key+'?')) return;
  try{await apiCall('resetHWID',{},key);toast('🔄 HWID direset','s');loadLicenses();}
  catch(e){toast('❌ '+e.message,'e');}
}
async function doDelete(key){
  if(!confirm('Hapus lisensi '+key+'? Tidak bisa dibatalkan!')) return;
  try{await apiCall('delete',{},key);toast('🗑️ Dihapus','e');loadLicenses();}
  catch(e){toast('❌ '+e.message,'e');}
}

function exportCSV(){
  if(!allLics.length){toast('Tidak ada data','e');return;}
  const h=['Kode','Nama','Tipe','Status','Aktivasi','Berlaku','Catatan'];
  const rows=allLics.map(l=>[l.key,l.name||'',l.type,l.status,l.activated_at?new Date(l.activated_at).toLocaleDateString('id-ID'):'',l.expires?new Date(l.expires).toLocaleDateString('id-ID'):'Seumur Hidup',l.note||''].map(v=>'"'+v+'"').join(','));
  const csv=[h.join(','),...rows].join('\\n');
  const blob=new Blob(['\\uFEFF'+csv],{type:'text/csv;charset=utf-8'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download='akpro-licenses.csv';a.click();
  toast('📥 CSV diunduh','s');
}
...
function toast(msg,type='i',dur=3500){
  ...
  t.className='tn '+type; t.textContent=msg;
